...
import { X, Anchor, Ship, Navigation, ArrowDown, TrendingUp, DollarSign, AlertTriangle } from 'lucide-react';
...
export default function PortPanel({ portName, initialData, onClose }: PortPanelProps) {
  ...
  return (
    <div className="fixed right-0 top-0 h-full w-full sm:w-[420px] bg-slate-900/95 backdrop-blur-sm border-l border-slate-700/50 z-[1000] overflow-y-auto shadow-2xl flex flex-col">
      {/* Header */}
        ...
        <div>
          ...
          {data && (
            <p className="text-xs text-slate-400 mt-0.5">
              {data.lat.toFixed(3)}°, {data.lon.toFixed(3)}° · Updated {new Date(data.lastUpdated).toLocaleTimeString()}
            </p>
          )}
        </div>
        <button
        ...
        <div className="p-4 flex flex-col gap-5">
          ...
          <div className="bg-slate-800/60 border rounded-xl p-4" style={{ borderColor: data.color + '55' }}>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs font-medium text-slate-400 uppercase tracking-wider">D&D Rate Recommendation</span>
            </div>
            ...
            <p className="text-xs text-slate-500 mt-2">
              Based on current {data.level.toLowerCase()} congestion. Base rate: $800/day.
            </p>
          </div>
          ...
          <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-3 text-center">
            <span className="text-xs text-slate-400">Total vessels in zone: </span>
            ...
            <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl overflow-hidden">
              <div className="p-3 border-b border-slate-700/50 flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-slate-400 uppercase tracking-wider">
                  Vessels in Zone ({data.vessels.length})
                </span>
              </div>
          ...
          {data.vessels.length === 0 && (
            <div className="text-center py-6 text-slate-500 text-sm">
              ...
              <p>No vessels detected in zone yet.</p>
              <p className="text-xs mt-1">Data builds up over the first 30 seconds.</p>
            </div>
          )}
        </div>
      )}
